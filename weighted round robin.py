...
# dans l'ordre croissant de leur soc et on attribue un poids en fonction de leur état de charge. La batterie ayant le
# poids le plus fort aura le plus d'énergie
...
Z1 = []
Z2 = []
panneaux = [k for k in range(80, 180, 1)]
capacites = [k for k in range(10, 110, 1)]
    ...
    for panneau in panneaux:
        ...
        for composants in range(nombre_de_foyers):
            ...
            batteries.append(pp.create_storage(net, bus=bus[nombre_de_foyers + composants], p_mw=0.00,
                                               ...
                                               min_e_mwh=0, max_e_mwh=0.0144 * capa, q_mvar=0, min_q_mvar=-0.1, max_q_mvar=0.1
        ...
        for pv in range(0, nombre_de_foyers):
            ...
            puissances_pvs.append(extraction_donnee(panneau)[0] * (1 / 1000))
        ...
        for timestep in range(0, len(timesteps)):
            ...
            for composant in range(0, nombre_de_foyers):  # calcul de la quantité d'énergie que recoit chaque batterie
                net.storage.at[nombre_de_foyers + composant, 'soc_percent'] += pot_commun * 100 / capacite / nombre_de_foyers
                n_recoit[composant] += 1
        ...
        # print(moyenne_coupure, moyenne_nrj, moyenne_std, moyenne_soc)
        ...
        liste_temps = [k for k in range(len(extraction_donnee(panneau)[0]))]
        ...
        liste_des_soc_simus.append(liste_soc_temps)
        Z1.append(float(moyenne_soc))
        Z2.append(moyenne_coupure)

# pano = panneaux
#
# for soc in range(0, nombre_de_foyers):
#
#     ax = plt.subplot()
#     ax.plot(liste_temps, liste_des_soc_simus[soc][0], label=str(pano[soc]) + 'panneaux solaire', color=colors[soc],
#              linestyle=line_styles[soc])
#     plt.xlabel('Temps (en j)')
#     plt.ylabel('SOC (en %)')
#     plt.legend(loc='upper left', prop={'size': 12})
#
#     lst_position = np.arange(144, len(extraction_donnee(pano[soc])[0]) + 144, 144)
#     l_xlabel = np.arange(1, 32)
#
...
CS1 = ax1.contourf(X, Y, Z1)
# ax1.scatter(X, Y)
ax1.clabel(CS1, inline=True, fontsize=20)
fig1.colorbar(CS1, ax=ax1, label='Interactive colorbar')
ax1.set_title('Valeur du SOC moyen sur le mois', fontsize=20)
ax1.set_xlabel('Nombre de panneaux solaire de 1.7 m²', fontsize=20)
ax1.set_ylabel('Nombre de batteries par foyer', fontsize=20)


fig2, ax2 = plt.subplots()
CS2 = ax2.contourf(X, Y, Z2)
# ax2.scatter(X, Y)
ax2.clabel(CS2, inline=True, fontsize=20)
fig2.colorbar(CS2, ax=ax2, label='Interactive colorbar')
ax2.set_title('Nombre de coupure sur le mois', fontsize=20)
ax2.set_xlabel('Nombre de panneaux solaire de 1.7 m²', fontsize=20)
ax2.set_ylabel('Nombre de batteries par foyer', fontsize=20)
